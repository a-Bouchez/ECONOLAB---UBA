model FirmasGoodsOrServices
  ...
  parameter Real epsilon(start = 0.55);   // Precio real de la energía
  // Pasar esto a Var para el shock de MarkUp (Mirar tmb FirmasGoods y FirmasServices)
  parameter Real theta(start = 0.4);      // Markup deseado
  // Pasar esto a Var para el shock de salario real (Mirar tmb FirmasServices)
  parameter Real OmegaT(start = 0.2);   // Salario real deseado (objetivo de trabajadores)
  parameter Real gamma(start = 0.1, fixed = true);
  
  input Real p_agregado;
  
  //PrecioConnector precio_venta; 
  //PrecioConnector precio_compra;
...
//USO DEL PRECIO DEL OTRO SECTOR
  pT = (1+theta)*(w+(delta*pTE)) + (gamma*precio_compra.p_valor);  // Precio objetivo de las empresas
  r = 1 - (w/p) - (delta*pTE/p) - (gamma * precio_compra.p_valor / p);   //Margen efectivo
  rT = theta/(1+theta);               // Margen objetivo
  hat_w = der(w)/w;                   // Crecimiento salarial nominal
  hat_p = der(p)/p;                   // Inflación
  pi = (p - w - pTE*delta - (gamma * precio_compra.p_valor)) / (p - pTE*delta - (gamma * precio_compra.p_valor)); // Cálculo del profit share par el gráfico
